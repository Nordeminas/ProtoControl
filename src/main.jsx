...
function ProtocolsView({ protocols, allProtocols, clients, employees, currentUser, company, search, setSearch, filters, setFilters, onSaveProtocol, onUpdateProtocol, onSaveClient, createdProtocol, setCreatedProtocol }) {
  ...
  return (
    <div className="grid">
      ...
      {detailsProtocol && <ProtocolDetailsModal protocol={detailsProtocol} currentUser={currentUser} onClose={() => setDetailsProtocol(null)} onSave={(updated) => { onUpdateProtocol(updated); setDetailsProtocol(updated); }} />}
      {showProtocolModal && (
...
function ProtocolDetailsModal({ protocol, currentUser, onClose, onSave }) {
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal large-modal">
        ...
        <div className="modal-actions"><button onClick={save}><CheckCircle2 size={18} /> Salvar alterações</button><button className="ghost" onClick={onClose}>Fechar</button></div>
      </div>
    </div>
  );
}
...
function PeopleManager({ title, type, people, onSave, protocols = [], includePassword = false }) {
  ...
  const submit = (event) => {
    ...
    const record = {
      ...
      ...(includePassword ? { role: existing?.role || 'employee', username: normalizeUsername(person.username || person.name) } : {}),
    };
    ...
    <div className="grid two-columns">
      <section className="card form-card">
        ...
        <form onSubmit={submit}>
          ...
          <div className="actions">
            <button className="primary" type="submit"><Plus size={18} /> {editingId ? 'Atualizar' : 'Salvar'}</button>
            {editingId && <button className="ghost" type="button" onClick={cancelEdit}>Cancelar</button>}
          </div>
        </form>
      </section>
      <section className="card">
        <div className="section-title"><div><span>Lista</span><h2>{title}</h2></div></div>
